...
13. [HƯỚNG DẪN BIÊN DỊCH, THIẾT LẬP & VẬN HÀNH HỆ THỐNG](#13-hướng-dẫn-biên-dịch-thiết-lập--vận-hành-hệ-thống)
    - [13.1 Hướng dẫn Biên dịch Native C++ Modules (CMake & MinGW-w64)](#131-hướng-dẫn-biên-dịch-native-c-modules-cmake--mingw-w64)
    - [13.2 Cài đặt Môi trường Phụ thuộc Python](#132-cài-đặt-môi-trường-phụ-thuộc-python)
    - [13.3 Khởi chạy Giao diện Desktop Chuyên dụng (`gui.py`)](#133-khởi-chạy-giao-diện-desktop-chuyên-dụng-guipy)
    - [13.4 Vận hành Công cụ Dòng lệnh Độc lập (CLI Tools)](#134-vận-hành-công-cụ-dòng-lệnh-độc-lập-cli-tools)
...
- **Launcher Động Thích Ứng**: Tạo script `Windows\System32hpai.cmd` sử dụng biến môi trường `%~d0` và `%SystemDrive%`, tự động thích ứng với ký tự ổ đĩa do WinPE gán.
...
## 13. HƯỚNG DẪN BIÊN DỊCH, THIẾT LẬP & VẬN HÀNH HỆ THỐNG

### 13.1 Hướng dẫn Biên dịch Native C++ Modules (CMake & MinGW-w64)
Hệ thống sử dụng CMake và MinGW-w64 (GCC 14.x / 16.x) liên kết với OpenSSL 3.x, Capstone và zlib:

```bash
# Tạo thư mục build và cấu hình CMake
cmake -B build -G "MinGW Makefiles" -DCMAKE_BUILD_TYPE=Release

# Biên dịch toàn bộ các thành phần Native cốt lõi:
cmake --build build --target pdf_analyzer pe_analyzer BHPAISandbox BHPAIMonitor bhr_identify bhpai_rescue -j4
```

---

### 13.2 Cài đặt Môi trường Phụ thuộc Python
Chuẩn bị môi trường Python 3.9+ (khuyên dùng Python 3.11):

```bash
# Cài đặt thư viện phụ thuộc
pip install -r requirements.txt
# Hoặc cài đặt các gói chính:
pip install PyQt6 capstone yara-python pefile scikit-learn lightgbm torch pandas numpy matplotlib psutil joblib cryptography
```

---

### 13.3 Khởi chạy Giao diện Desktop Chuyên dụng (`gui.py`)
...
### 13.4 Vận hành Công cụ Dòng lệnh Độc lập (CLI Tools)
...
```bash
./pe_analyzer.exe path/to/sample.exe --export-json report.json --gen-yara
```

#### 3. Khởi chạy Dynamic Isolated Sandbox:
```bash
./BHPAISandbox.exe path/to/malware.exe
```

#### 4. Vận hành Cứu hộ Khẩn cấp WinRE:
```powershell
# Nhúng bộ công cụ vào WinRE (PowerShell Administrator):
.\scripts\deploy_winre.ps1

# Kích hoạt chu kỳ boot tiếp theo vào môi trường cứu hộ:
.\scripts	rigger_rescue_reboot.ps1
```
